...
**Note**: the queryparameters `page` and `all` are undocumented in this schema
(which is fine because the API clients we generate generally won't want paginated results).
When a query has the parameter `all=false` or `page=#` where `#` is a number, 
the result is a paginated one with `rest_framework.pagination.ageNumberPagination`:
results are returned in an object like
`{prev: 'link-to-prev-page', next: 'link-to-next-page', count: 'result-count', results: []}`
