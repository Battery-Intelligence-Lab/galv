...
def import_file_direct(core_path: str, file_path: str) -> bool:
    """
        Attempts to import a given file
    """
    default_column_ids = get_standard_columns()
    ...
    if not os.path.isfile(full_file_path):
        ...
        report_harvest_result(path=core_path, file=file_path, error=FileNotFoundError())
        return False
    logger.info("Importing " + full_file_path + " [direct]")
    ...
    try:
        ...
        logger.info(f"Loaded file in {time.process_time() - load_start}")
        # Send metadata
        load_meta = time.process_time()
        core_metadata, extra_metadata = input_file.load_metadata()
        logger.info(f"Loaded metadata in {time.process_time() - load_meta}")
        report = report_harvest_result(
            path=core_path,
            file=file_path,
            content={
        ...
        logger.info(f"Loaded and parsed data in {time.process_time() - load_data}")
        # Send data
        send_data = time.process_time()
        report = report_harvest_result(path=core_path, file=file_path, content={
            'task': 'import',
            'status': 'in_progress',
            'data': [v for v in column_data.values()],
        ...
        logger.info(f"Sent data in {time.process_time() - send_data}")
        if report is None:
            logger.error(f"API Error")
            return False
        if not report.ok:
