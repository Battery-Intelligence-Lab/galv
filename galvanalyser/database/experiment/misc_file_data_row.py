...
class MiscFileDataRow:
    def __init__(self, dataset_id, lower_sample_range, upper_sample_range, key, json_data=None, binary_data=None):
    ...
    def insert(self, conn):
        with conn.cursor() as cursor:
            cursor.execute(
                ...
                [self.dataset_id, self.lower_sample_range,
                self.upper_sample_range , self.key,
    ...
    def select_from_dataset_id(dataset_id, conn):
        with conn.cursor() as cursor:
            ...
            return [
                MiscFileDataRow(dataset_id=dataset_id,
                lower_sample_range=result[0].lower,
                upper_sample_range=result[0].upper,
                key=result[1],
                json_data=json.loads(result[2]) if result[2] is not None else None,
                binary_data=bytes(result[3]) if result[3] is not None else None)
