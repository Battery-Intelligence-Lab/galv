...
const MonitoredPaths = jest.requireActual('../MonitoredPaths').default;
...
it('MonitoredPaths has appropriate columns', async () => {
    ...
    await act(async () => render(<MonitoredPaths harvester={mock_harvester}/>));
...
describe('MonitoredPaths', () => {
    ...
    beforeEach(async () => {
        ...
        let container
        await act(async () => container = render(<MonitoredPaths harvester={mock_harvester}/>).container);
    })
    ...
    it('has appropriate values', async () => {
        ...
        expect(await screen.findAllByDisplayValue(mock_paths[0].path)).not.toHaveLength(0);
        expect(await screen.findByDisplayValue(mock_paths[0].regex)).toBeInTheDocument();
    });
    ...
    it('sends an API call when a new Path is created', async () => {
        ...
        await act(async () => {
            await user.type(screen.getAllByPlaceholderText(/path$/)[mock_paths.length], new_path);
            await user.type(screen.getAllByPlaceholderText(".*")[mock_paths.length], '{Backspace>5}^[[^T]');
            await user.type(screen.getAllByDisplayValue(
                mock_paths[0].stable_time)[mock_paths.length],
    ...
    it('sends an update API call when saved', async () => {
        ...
        await act(async () => {
            const name = screen.getAllByPlaceholderText('/new/directory/path')[0]
            await user.type(name, `{Backspace>50}${new_path}`)
            await user.click(screen.getAllByLabelText(/^Save$/)[0])
        });
        expect(mocked_fetch).toHaveBeenCalledWith(
            mock_paths[0].url,
            {
