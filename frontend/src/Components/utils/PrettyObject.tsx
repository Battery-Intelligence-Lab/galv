...
import {TypeChangerSupportedType} from "./TypeChanger";

export type PrettyObjectProps = {
    target: { [key: string]: TypeChangerSupportedType }
    exclude_keys?: string[]
...
function rename_key_in_place(
    obj: PrettyObjectProps["target"],
    old_key: string,
    new_key: string
): PrettyObjectProps["target"] {
...
export default function PrettyObject(
    ...
    const edit_fun_factory = (k: string) => (v: TypeChangerSupportedType) => _onEdit({...target, [k]: v})
                                    ...
                                    {_edit_mode && !_type_locked_keys.includes(key) ?
                                        <Prettify
                                            ...
                                            onEdit={(new_key) => {
                                                ...
                                                _onEdit!(rename_key_in_place(target, key, new_key))
                                            }}
                                            target={key}
                                            label="key"
                                        /> :
                    ...
                    {_edit_mode && <TableRow key="add_new">
                        <TableCell component="th" scope="row" key="add_key" align="right">
                            <Prettify
                                ...
                                label="+ KEY"
                                multiline={false}
                                onEdit={(new_key: TypeChangerSupportedType) => {
                                    // Add new key
